...
class TestStereo3DDetConfig:
    ...
    def test_backbone_structure_matches_yolo11_obb(self):
        ...
        assert len(stereo_backbone) == len(obb_backbone), (
            f"Backbone layer count mismatch: stereo has {len(stereo_backbone)}, yolo11-obb has {len(obb_backbone)}"
        )
        ...
        assert first_layer_module == "StereoConv", f"First layer should use StereoConv, got {first_layer_module}"
        ...
        for i in range(1, len(stereo_backbone)):
            ...
            assert stereo_layer == obb_layer, (
                f"Layer {i} mismatch:\n  stereo3ddet: {stereo_layer}\n  yolo11-obb:  {obb_layer}"
            )

        print("✓ Backbone structure matches yolo11-obb.yaml")
        ...
        print("  - Layers 1-10: Match yolo11-obb.yaml exactly")
        print("  - Layer structure verified: ✓ (T022 requirement met)")
    ...
    def test_pan_neck_structure_matches_yolo11_obb(self):
        ...
        for i, layer in enumerate(stereo_head):
            ...
            if isinstance(layer, list) and len(layer) >= 3:
                module_name = (
                    ...
                    else (layer[2].__name__ if hasattr(layer[2], "__name__") else str(layer[2]))
                )
        ...
        assert len(stereo_pan_neck) >= 12, f"PAN neck should have at least 12 layers, got {len(stereo_pan_neck)}"
        assert len(obb_pan_neck) == 12, f"yolo11-obb.yaml PAN neck should have 12 layers, got {len(obb_pan_neck)}"
        ...
        def get_layer_info(layer):
            ...
            return {"from": from_idx, "repeats": repeats, "module": module_name, "args": args}
        ...
        for i in range(6):
            ...
            # Layer 1, 4: Concat
                ...
                assert stereo_info["module"] == "Concat", f"Layer {i}: Should be Concat, got {stereo_info['module']}"
            ...
            # Layer 2, 5: C3k2
                ...
                assert stereo_info["module"] == "C3k2", f"Layer {i}: Should be C3k2, got {stereo_info['module']}"
        ...
        for i in range(6, 12):
            ...
            if i in [6, 9]:
                ...
                assert stereo_info["module"] == "Conv", f"Layer {i}: Should be Conv, got {stereo_info['module']}"
                # Verify Conv args: [channels, kernel, stride]
                stereo_args = stereo_info["args"]
                obb_args = obb_info["args"]
                if len(stereo_args) >= 2 and len(obb_args) >= 2:
                    # Verify kernel and stride match (3, 2)
                    assert stereo_args[1] == obb_args[1] == 3, f"Layer {i}: Conv kernel size should be 3"
                    assert stereo_args[2] == obb_args[2] == 2, f"Layer {i}: Conv stride should be 2"
        ...
        print("  - Top-down path: 6 layers ✓")
        print("  - Bottom-up path: 6 layers ✓")
...
class TestStereo3DDetConfigHead:
    ...
    def test_head_uses_stereo_centernet_head_in_config(self):
        ...
        print("✓ Head uses StereoCenterNetHead (not Detect)")
    ...
    def test_head_uses_stereo_centernet_head_in_model(self):
        ...
        try:
            ...
            print("✓ Model uses StereoCenterNetHead (not Detect)")
            ...
            print("  (This is acceptable - config structure test passed)")
...
class TestStereo3DDetModelInstantiation:
    ...
    def test_model_can_be_instantiated_from_config(self):
        ...
        print("✓ Model instantiated via YOLO API")
        ...
        print("✓ Model instantiated via Stereo3DDetModel directly")
        ...
        print("✓ Model instantiated from config path string")
        print("  - Config path handling: ✓")
...
class TestStereo3DDetForwardPass:
    ...
    def test_model_forward_pass_produces_10_branch_output(self):
        ...
        for branch_name, expected_shape in expected_shapes.items():
            ...
            assert isinstance(outputs[branch_name], torch.Tensor), f"Branch '{branch_name}' output is not a tensor"

        print("✓ Model forward pass produces 10-branch output")
        ...
        print("  - All branch shapes verified: ✓")
        print("  - T026 requirement met: ✓")
