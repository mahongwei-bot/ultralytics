...
class TestPerspectiveKeypoints:
    ...
    def test_batch_version_single_object(self):
        ...
        vertices_single = torch.tensor([[-0.5, -0.5], [0.5, -0.5], [0.5, 0.5], [-0.5, 0.5]], dtype=torch.float32)
    ...
    @pytest.mark.parametrize(
        "orientation,expected_indices,expected_name",
        [
            (-3 * np.pi / 4, (0, 1), "front"),
            (-np.pi / 4, (1, 2), "right"),
            (np.pi / 4, (2, 3), "rear"),
    ...
    def test_orientation_normalization_out_of_range(self, sample_vertices_numpy):
        ...
        kp1_normal, kp2_normal = select_perspective_keypoints(sample_vertices_numpy, orientation_normal)
        kp1_wrapped, kp2_wrapped = select_perspective_keypoints(sample_vertices_numpy, orientation_wrapped)
...
class TestOcclusionClassification:
    ...
    Tests the depth-line based occlusion classification algorithm from Algorithm 1 "3D Object Classification Strategy"
    in the Stereo CenterNet paper.

    The algorithm classifies objects as occluded or unoccluded based on whether their left and right boundaries are
    visible (not blocked by closer objects).
    """
    ...
    def test_object_at_image_left_edge(self):
        ...
        _occluded, unoccluded = classify_occlusion(detections, image_width=400)
    ...
    def test_depth_tolerance_parameter(self):
        ...
        _occluded_default, unoccluded_default = classify_occlusion(detections, image_width=400, depth_tolerance=1.0)
        ...
        occluded_strict, unoccluded_strict = classify_occlusion(detections, image_width=400, depth_tolerance=0.1)
...
class TestOcclusionIntegration:
    ...
    def test_full_pipeline_occlusion_to_dense_alignment_decision(self):
        ...
        occluded, _unoccluded = classify_occlusion(detections, image_width=400)
...
class TestGeometricConstruction:
    ...
    def known_geometry(self) -> tuple[GeometricObservations, tuple[float, float, float, float]]:
        ...
        Returns:
            Tuple of (observations, expected_solution) where expected_solution: is (x, y, z, theta).
        """
    ...
    def test_convergence_known_geometry(self, solver, known_geometry):
        ...
        obs, (_x_true, _y_true, z_true, theta_true) = known_geometry

        _x, _y, z, _theta, _converged = solver.solve(
            observations=obs,
    ...
    def test_convergence_various_depths(self, solver):
        ...
        for z_true in test_depths:
            ...
            _x, _y, z, _theta, converged = solver.solve(
                observations=obs,
                dimensions=self.DEFAULT_DIMS,
                theta_init=theta_true,
                calib=calib,
    ...
    def test_convergence_various_orientations(self, solver):
        ...
        for theta_true in test_orientations:
            ...
            _x, _y, _z, theta, converged = solver.solve(
                observations=obs,
                dimensions=self.DEFAULT_DIMS,
                theta_init=theta_true,
                calib=calib,
    ...
    def test_convergence_rate_target_sc007(self):
        ...
        for i in range(num_tests):
            ...
            _x, _y, z, _theta, _converged = solver.solve(obs, self.DEFAULT_DIMS, theta_true, calib)
        ...
        assert valid_results == num_tests, f"All samples should produce valid depths, got {valid_results}/{num_tests}"
    ...
    def test_numerical_stability_small_disparity(self, solver):
        ...
        obs = GeometricObservations(
            ...
            vl=180.0,
            ur=599.0,
            vr=180.0,  # Only 1 pixel disparity
            ul_prime=599.0,
            ur_prime=599.0,
            up=605.0,
            vp=182.0,
        ...
        x, y, z, theta, _converged = solver.solve(
            observations=obs,
            dimensions=self.DEFAULT_DIMS,
            theta_init=0.0,
            calib=calib,
    ...
    def test_numerical_stability_zero_disparity(self, solver):
        ...
        obs = GeometricObservations(
            ...
            vl=180.0,
            ur=600.0,
            vr=180.0,  # Same position = 0 disparity
            ul_prime=600.0,
            ur_prime=600.0,
            up=610.0,
            vp=185.0,
    ...
    def test_fallback_invalid_disparity(self):
        ...
        _x, _y, z, _theta = fallback_simple_triangulation(
    ...
    def test_batch_processing_multiple_detections(self):
        ...
        refined, _convergence_rate = solve_geometric_batch(detections, calib)
    ...
    def test_batch_processing_with_fallback(self):
        ...
        refined, _ = solve_geometric_batch(detections, calib, fallback_on_failure=True)
    ...
    def test_batch_processing_dict_calib(self):
        ...
        # Should not raise
        refined, _convergence_rate = solve_geometric_batch(detections, calib_dict)
        assert len(refined) == 1
    ...
    def test_single_detection_with_keypoint(self):
        ...
        _x, _y, z, _theta, _converged = solve_geometric_single(
            center_2d=(600.0, 180.0),
            disparity=20.0,
            dimensions=self.DEFAULT_DIMS,
            theta_init=0.0,
    ...
    def test_residuals_computation(self, solver):
        ...
        residuals = solver._compute_residuals(state, obs, self.DEFAULT_DIMS, self.DEFAULT_CALIB)
    ...
    def test_jacobian_computation(self, solver):
        ...
        jacobian = solver._compute_jacobian(state, obs, self.DEFAULT_DIMS, self.DEFAULT_CALIB)
...
class TestBackwardCompatibility:
    ...
    These tests ensure that existing stereo 3D detection models continue to work correctly after the addition of new
    features (NMS, geometric construction, dense alignment, occlusion classification, DLA-34 backbone, uncertainty
    weighting).
    ...
    def test_geometric_solver_api_contract(self):
        ...
        _x, _y, _z, _theta, converged = result
    ...
    def test_geometric_fallback_on_failure(self):
        ...
        refined, _convergence_rate = solve_geometric_batch(detections, calib, fallback_on_failure=True)
    ...
    def test_images_accept_numpy_and_tensor(self):
        ...
        result_tensor = aligner.refine_depth(left_tensor.numpy(), right_tensor.numpy(), box3d, calib)
