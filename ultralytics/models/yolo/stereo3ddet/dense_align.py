...
class DenseAlignment:
    ...
    This class implements the dense alignment stage from Stereo CenterNet. Given an initial 3D bounding box estimate
    from geometric construction, it refines the depth using photometric matching between stereo images.

    Paper Reference: Section 3.2 "Dense Alignment"

    The algorithm searches over depth hypotheses around the initial estimate, computing photometric error for each
    hypothesis by warping the right image patch to the left view and comparing with the left image patch.
    ...
    def __init__(
        ...
        Args:
            depth_search_range: Search range (meters) around initial depth estimate. The algorithm searches from (z_init
                - range) to (z_init + range). Default 2.0m provides good coverage for typical KITTI scenarios.
            depth_steps: Number of depth hypotheses to evaluate within search range. Higher values give finer depth
                resolution but increase computation. Default 32 provides ~12.5cm resolution with 2.0m range.
            patch_size: Size of square patches for photometric matching (pixels). Larger patches are more robust but
                slower. Default 7 is a good balance.
    ...
    def _ncc_error(
        ...
        Args:
            ...
            p2: Second image patch, shape [H, W] or [H, W, C]. Must have same shape as p1.

        Returns:
            NCC error (negative of correlation coefficient).
            Range: [-1, 1] where -1 means perfect match (best), 1 means anti-correlation. Returns infinity if patches
                have zero variance.
    ...
    def _project_box_to_roi(
        ...
        Returns:
            (x1, y1, x2, y2): ROI bounding box in pixel coordinates. Coordinates are clamped to valid image bounds [0,
                image_size].
        ...
        u_coords = (
            fx * np.divide(corners_x_world[valid_mask], z_valid, out=np.zeros_like(z_valid), where=z_valid != 0) + cx
        )
        v_coords = (
            fy * np.divide(corners_y_world[valid_mask], z_valid, out=np.zeros_like(z_valid), where=z_valid != 0) + cy
        )
    ...
    def _extract_patch(
        ...
        Args:
            ...
            roi: Region of interest (x1, y1, x2, y2) in pixel coordinates. Negative coordinates are valid (will be
                zero-padded).
    ...
    def _warp_right_to_left(
        ...
        Args:
            ...
            disparity: Stereo disparity in pixels (fx * baseline / depth). Positive value indicates right image shifts
                left.
    ...
    def refine_depth(
        ...
        for z in z_candidates:
            ...
            if len(warped_right.shape) == 3 and warped_right.shape[2] == 3:
                warped_right_gray = (
                    0.299 * warped_right[:, :, 2] + 0.587 * warped_right[:, :, 1] + 0.114 * warped_right[:, :, 0]
                ).astype(np.float32)
            else:
    ...
    def refine_depth_batch(
        ...
        """
        return [self.refine_depth(left_img, right_img, box, calib) for box in boxes3d_init]
