...
def select_perspective_keypoints(
    ...
    This function implements the perspective keypoint selection algorithm from Section 3.2 of the Stereo CenterNet
    paper. Given the 4 bottom vertices of a 3D bounding box and the object's orientation (yaw angle), it selects the 2
    vertices that are most visible from the camera viewpoint.
    ...
    Args:
        ...
        orientation: Object yaw angle θ in radians, range [-π, π]. θ=0 means facing away from camera (rear visible)
            θ=π/-π means facing toward camera (front visible) θ=π/2 means left side visible θ=-π/2 means right
            side visible

    Returns:
        (kp1, kp2): Tuple of two keypoint coordinates, each as [2] array/tensor. These are the two visible bottom
            corners based on orientation.
...
def select_perspective_keypoints_batch(
    ...
    This function performs the same keypoint selection as `select_perspective_keypoints` but efficiently handles batches
    of objects in a single forward pass without Python loops.
    ...
    Returns:
        (kp1, kp2): Tuple of two tensors, each [N, 2] containing the selected keypoints for each object in the batch.
...
def get_visible_face_indices(orientation: float | torch.Tensor) -> tuple[int, int]:
    """Get the indices of the two visible vertices based on orientation.

    This is a utility function that returns just the vertex indices without requiring the actual vertex coordinates.
...
def get_quadrant_name(orientation: float | torch.Tensor) -> str:
