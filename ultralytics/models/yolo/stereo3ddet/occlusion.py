...
def should_skip_dense_alignment(
    ...
    Dense photometric alignment can produce incorrect results for heavily occluded objects because the occluding
    object's appearance will contaminate the photometric error measurement. This helper function determines whether to
    skip dense alignment based on occlusion classification.

    Args:
        detection_idx: Index of the detection to check.
        occluded_indices: List of detection indices classified as occluded (typically from classify_occlusion()).

    Returns:
        bool: True if dense alignment should be skipped for this detection (i.e., the detection is heavily occluded),
            False otherwise.
