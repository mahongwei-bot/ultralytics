...
class Stereo3DDetModel(DetectionModel):
    ...
    For now this extends the standard DetectionModel so we can train/eval the stereo3ddet task end-to-end while we
    iterate on a dedicated stereo head/loss.
    """
            ...
            if not hasattr(self, "_stereo_criterion"):
                ...
                if "targets" in batch and isinstance(batch["targets"], dict):
                    ...
                    if "heatmap" in batch["targets"]:
                        num_classes = (
                            batch["targets"]["heatmap"].shape[1] if len(batch["targets"]["heatmap"].shape) > 1 else 3
                        )
